...
public class Solution {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetingslist) {
        
       Object[] arr=new Object[48];
        for(int i=0;i<meetingslist.size();i++){
            int st=meetingslist.get(i).startTime;
            for(int j=st;j<meetingslist.get(i).endTime;j++){
                arr[j]=Boolean.TRUE;
            }
        }
        List<Meeting> result=new ArrayList<>();
        int flag=0,st=0,et=0;
        
        for(int k=0;k<arr.length;k++){
            if(arr[k] == Boolean.TRUE && flag == 0){
                flag=1;
                st=k;
            } else if(arr[k]==null && flag == 1){
                flag=0;
                et=k;
                Meeting meeting=new Meeting(st,et);
                result.add(meeting);
            }
        }
        return result;
